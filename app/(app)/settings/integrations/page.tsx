            ...
            <Card key={status.name}>
              <CardHeader>
                <div className="flex items-center justify-between gap-3">
                  ...
                  <Badge className={status.configured ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800"}>{status.mode}</Badge>
                </div>
              </CardHeader>
              <CardContent className="space-y-3">
                <p className="text-sm leading-6 text-slate-600">{status.description}</p>
                <p className="rounded-2xl bg-slate-50 p-4 text-sm font-medium text-slate-700">{status.nextStep}</p>
              </CardContent>
            </Card>
          ))}
        </div>
