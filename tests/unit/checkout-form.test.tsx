...
const { pushMock, assignMock, signInMock } = vi.hoisted(() => ({
  pushMock: vi.fn(),
  assignMock: vi.fn(),
  signInMock: vi.fn()
}));
...
function fillRequiredFields() {
  ...
  fireEvent.change(screen.getByLabelText("Nome da empresa"), { target: { value: "ZapBurger Centro" } });
...
describe("checkout form", () => {
  ...
  it("marks required checkout fields", () => {
    ...
    expect(screen.getByLabelText("Nome da empresa").getAttribute("required")).not.toBeNull();
  ...
  it("blocks submit when passwords do not match", async () => {
    ...
    fireEvent.submit(screen.getByRole("button", { name: "Comecar agora" }).closest("form")!);
  ...
  it("shows a safe message when account creation succeeds but sign-in fails", async () => {
    vi.mocked(fetch).mockResolvedValueOnce(
      new Response(
        JSON.stringify({
          checkoutUrl: "https://sandbox.asaas.com/i/pay_123",
    ...
    await screen.findByText("Refaca o envio para retomar o checkout com sua sessao autenticada.");
    expect(assignMock).not.toHaveBeenCalled();
  });
});
